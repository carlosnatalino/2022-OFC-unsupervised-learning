...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut client_ip: &String = &String::from("localhost");
    let num_reqs = 30;
    ...
    let mut num_clients = 30;
    if args.len() > 2 {
        num_clients = args[2].parse::<i32>().unwrap();
    }
    ...
    for ida in 1i32..num_reqs {
        ...
        for _cli in 1..num_clients {
            ...
            println!("RESPONSE,{},{}", ida, now.elapsed().as_millis());
            // println!("Length: {}", response.into_inner().cluster_indices.len());
        }
        thread::sleep(Duration::from_millis(30000 - sum_grpc as u64));
    }
